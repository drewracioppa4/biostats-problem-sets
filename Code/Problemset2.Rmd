...
View(DF)
```
It was a bad choice to delete these rows because you are throwing away data. For instance, row 4 has data for everything except age so why get rid of all of the other information?
...
print(Plot.DF)
```
I'm not sure what the `print(Plot.DF)` line is for

Also, you are now missing observations because of deleting them from the data frame. If you had kept them, you would simply need to add `na.rm =T` inside of your summarise statement for both means and it would work just fine. See key for details.
...
```
The filter to create Plot.DF2 didn't work because there are no cases in DF where "not scary" is spelled with all lowercase so R returns an empty dataframe.
